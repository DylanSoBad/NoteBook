...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (raw.length > 2048) return back('invalid');
    const password = new URLSearchParams(raw).get('password') || '';
    if (!password || password.length > 256) return back('invalid');
    // Vercel overwrites x-real-ip; client-supplied Sites identity headers are ignored.
    ...
    if (!await verifyPassword(password)) return back('invalid');
