...
export function validateWriterInput(value: unknown): WriterInput {
  if (!value || typeof value !== 'object')
    throw new WriterError('Hãy nhập ý tưởng để AI viết bài.', 400);
  const v = value as WriterInput;
  if (
    typeof v.brief !== 'string' ||
    v.brief.trim().length < 10 ||
    v.brief.length > 3000
  )
    throw new WriterError('Ý tưởng cần từ 10 đến 3.000 ký tự.', 400);
  if (typeof v.facts !== 'string' || v.facts.length > 6000)
    throw new WriterError('Thông tin bổ sung tối đa 6.000 ký tự.', 400);
  if (!aiTones.includes(v.tone) || !aiTopics.includes(v.topic))
    throw new WriterError('Chọn giọng viết và chủ đề hợp lệ.', 400);
  return {
    brief: v.brief.trim(),
    facts: v.facts.trim(),
    tone: v.tone,
...
Write THREE distinct short-post alternatives for the same brief, not three sequential parts of a thread. Each must stand alone. Use sharp natural English, a specific hook and a human point of view. No corporate hype, forced hashtags, engagement bait, generic motivational filler or promises of follower growth. Degen means playful crypto-native language, never reckless investment instructions or guaranteed gains. Builder means practical observations, trade-offs and useful lessons. Mixed tone balances both. Use at most one emoji per draft and aim for 180-240 characters per body, never more than 280 characters. Titles are short internal labels, not part of the post. PERSONAL HISTORY RULE: An opinion brief does NOT authorize a story about Dylan's past. Do not write 'I stopped', 'I started', 'I tested', 'I spent', 'I earned', 'I met', 'I learned' or any equivalent personal action unless that precise experience is explicitly supplied. For an opinion use 'My take:' or a general observation instead. Do not promise the reader better outcomes such as making fewer mistakes, sleeping better or guaranteed confidence; discuss trade-offs and possibilities instead.
If the input includes an example post, use it only as raw source material. Extract the underlying topic, facts and tension, then create a genuinely new thesis, hook, structure, rhythm and wording. Do not paraphrase sentence by sentence, mirror the source's cadence, preserve signature phrases, or reuse six or more consecutive words except proper nouns, handles, URLs and unavoidable technical terms. The three alternatives must explore different angles, not synonym swaps. Never attribute the source author's experience or opinion to Dylan unless the input explicitly says it is Dylan's own.
...
export function parseDrafts(value: unknown): AIDraft[] {
  ...
  if (!Array.isArray(list) || list.length !== 3)
    throw new WriterError('AI chưa trả về đủ bản nháp. Hãy thử lại.');
  return list.map((v) => {
    if (
      !v ||
      ...
      !v.body.trim() ||
      Array.from(v.body).length > 280
    )
      throw new WriterError(
        'Bản nháp AI chưa đúng định dạng bài ngắn. Hãy thử lại.',
      );
...
export async function generateDrafts(
  ...
  if (gemini) headers['x-goog-api-key'] = credential.token;
  ...
  try {
    ...
    throw new WriterError(
      'AI chưa phản hồi kịp. Ý tưởng vẫn ở đây; bạn có thể thử lại.',
      504,
      'timeout',
    );
  }
  if (!response.ok) {
    ...
    if (response.status === 401 || response.status === 403)
      throw new WriterError(
        'API key AI chưa đúng hoặc chưa được cấp quyền. Kiểm tra lại key bạn đang dùng.',
        503,
        'configuration',
      );
    if (response.status === 402)
      throw new WriterError(
        'Tài khoản API chưa có đủ credit. Kiểm tra thanh toán tại nhà cung cấp AI.',
        503,
        'activation_required',
      );
    if (response.status === 429)
      throw new WriterError(
        'Dịch vụ AI đang giới hạn lượt dùng hoặc đã hết quota. Vui lòng kiểm tra credit và thử lại sau.',
        429,
        'provider_limit',
      );
  ...
  try {
    ...
    if (gemini) {
      ...
      if (
        ...
        throw new WriterError(
          'AI không thể viết theo yêu cầu này. Bạn hãy đổi cách mô tả.',
          422,
          'refusal',
        );
      ...
      const text = (candidate.content?.parts || [])
        .filter(
          (v: { text?: string; thought?: boolean }) =>
            typeof v.text === 'string' && !v.thought,
        )
    ...
    const parts = data.output
      .filter((v: { type: string }) => v.type === 'message')
      .flatMap((v: { content: unknown[] }) => v.content || []);
    if (parts.some((v: { type: string }) => v.type === 'refusal'))
      throw new WriterError(
        'AI không thể viết theo yêu cầu này. Bạn hãy đổi cách mô tả.',
        422,
        'refusal',
    ...
    if (e instanceof WriterError) throw e;
    throw new WriterError('Chưa đọc được bản nháp từ AI. Hãy thử lại.');
  }
}
